...
const currencies = [
  ...
  'CAD', // Canada
  'DKK', // Denmark
  'EUR', // France, Germany, Ireland, Italy, Belgium, Euro
  'HKD', // Hong Kong
  'JPY', // Japan
  'MXN', // Mexico
  'MAD', // Morocco
...
gulp.task('cldr', () => {
  const cldrFilter = filter(
    file => (locales.indexOf(
      path
        .dirname(file.path)
        .split(path.sep)
        .pop(),
    ) > -1
        && files.indexOf(path.basename(file.path, '.json')) > -1)
      || (path
        .dirname(file.path)
        .split(path.sep)
        .pop() === 'supplemental'
        && supplemental.indexOf(path.basename(file.path, '.json')) > -1),
  );

  return gulp
    .src([
      './node_modules/cldr-data/supplemental/*.json',
      './node_modules/cldr-data/main/**/*.json',
    ])
    .pipe(cldrFilter)
    .pipe(
      merge({
